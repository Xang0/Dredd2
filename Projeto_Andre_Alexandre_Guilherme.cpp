...
void leitura(fstream& arquivo, pessoa cadastros[], int tam, bool& leitura_correta){
    char lixo;
    string lixao;
...
void cadastrarNovaPessoa(bool& continuar, int& linhas, pessoa cadastros[]) {
    ...
    cout << "CPF com '.' e '-': " << endl;
    cin >> novaPessoa.cpf;
    if (strlen(novaPessoa.cpf) != 14) {//Verifica se CPF digitado com 11 números mais os pontos e o hífens
    ...
    cin >> novaPessoa.tipoSanguineo;
    for (size_t i = 0; i < strlen(novaPessoa.tipoSanguineo); ++i) {//Transforma a letra digitada em maiúsculo
    novaPessoa.tipoSanguineo[i] = toupper(novaPessoa.tipoSanguineo[i]);
}
    //Verifica se é um tipo sanguineo válido
    bool sentinela = true;
    unsigned auxiliar = 0;
    while(auxiliar < strlen(novaPessoa.tipoSanguineo)){
        if (novaPessoa.tipoSanguineo[auxiliar] != 'A' and novaPessoa.tipoSanguineo[auxiliar] != 'B' and novaPessoa.tipoSanguineo[auxiliar] != 'O' and
            novaPessoa.tipoSanguineo[auxiliar] != '+' and novaPessoa.tipoSanguineo[auxiliar] != '-')
        {
            sentinela = false;
        }
        auxiliar++;
    ...
    if(!sentinela){
        cout << "Esse tipo sanguineo não existe.";
        return;
    }
    ...
    unsigned contador = 0;
    while(contador < strlen(novaPessoa.nome)){
        cadastros[linhas-1].nome[contador] = novaPessoa.nome[contador];
        contador++;
    }
    contador = 0;
    while(contador < strlen(novaPessoa.cpf)){
        cadastros[linhas-1].cpf[contador] = novaPessoa.cpf[contador];
        contador++;
    }
    contador = 0;
    while(contador < strlen(novaPessoa.tipoSanguineo)){
        cadastros[linhas-1].tipoSanguineo[contador] = novaPessoa.tipoSanguineo[contador];
        contador++;
    }
...
void alterarpessoa(pessoa nomeProcurado[], int linhas, bool& continuar) {
    ...
    if (encontrar) {
        ...
        for(int i=0; i<linhas ; i++){ //Faz a procura em todo o vetor para saber se o CPf já é cadastrado
            if(i != indice and nomeProcurado[indice].cpf == nomeProcurado[i].cpf){
                cout<< "Esse CPF já existe" << endl;
        return;
        }
    }
        ...
        if (nomeProcurado[indice].porcentVermelho + nomeProcurado[indice].porcentBranco > 100) { //Verifica se a soma das porcentagens de glóbulos brancos e veremlhos não passa de 100
            cout << "A soma das porcentagens de glóbulos vermelhos e brancos deve ser igualo ou menor que 100%." << endl;
        return;
        }
        cout << "Densidade: ";
        ...
        if (!arquivo) {
            ...
            //De linhas é subtraido 2 pela variavel i estar sendo usada para percorrer o vetor
...
int main() {
    ...
    fstream arquivo("dados_hemograma.csv", ios::binary);
